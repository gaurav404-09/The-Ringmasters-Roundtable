...
const AutocompleteInput = ({ value, onChange, placeholder, className = '' }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (input.length > 1) {
      ...
      console.log(`Searching for: "${inputLower}"`);
      
      // Debug: Check if Deoria is in the cities array
      const deoriaExists = cities.some(city => 
        city.toLowerCase().includes('deoria')
      );
      console.log(`Does 'Deoria' exist in cities? ${deoriaExists ? 'Yes' : 'No'}`);
      
      const filtered = cities
        .filter(city => {
          const cityLower = city.toLowerCase();
          const cityName = city.split(',')[0].toLowerCase().trim();
          const matches = cityLower.includes(inputLower) || cityName.includes(inputLower);
          
          // Debug: Log matches for 'deoria'
          if (inputLower === 'deoria' && matches) {
      ...
      console.log(`Found ${filtered.length} matches for "${inputLower}"`);
  ...
  const formatSuggestion = (suggestion, input) => {
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="truncate">
                    {formatSuggestion(suggestion, value.toLowerCase())}
                  </span>
                </div>
              </li>
            ))}
